...
describe('Output helpers', () => {
  ...
  describe('has method parseEmptyLine', () => {
    ...
    test('returns string as an output', () => {
      expect(typeof outputHelpers.parseEmptyLine(HAPPY_PATH_DEFAULTS)).toBe(
        'string',
      );
    });
    ...
    describe('with default settings', () => {
      test('returns 60 asterisks ("*" x 60)', () => {
        expect(outputHelpers.parseEmptyLine(HAPPY_PATH_DEFAULTS)).toMatch(
          /\*{60}/gi,
        );
      });
  ...
  describe('has mehthod parseComment', () => {
    ...
    test('returns string as an output', () => {
      expect(
        typeof outputHelpers.parseComment(HAPPY_PATH_DEFAULTS, TEST_COMMENT),
      ).toBe('string');
    });
    ...
    describe('with default settings', () => {
      test('contains comment passed as parameter', () => {
        expect(
          outputHelpers.parseComment(HAPPY_PATH_DEFAULTS, TEST_COMMENT),
        ).toContain(TEST_COMMENT);
      });
      ...
      test('returns inside console.log 60 chars', () => {
        expect(
          outputHelpers.parseComment(HAPPY_PATH_DEFAULTS, TEST_COMMENT),
        ).toMatch(/^console\.log\('.{60}'\);$/gi);
      });

      test('returns "console.log(\'" (with singlequote) on the beginning of string', () => {
        expect(
          outputHelpers.parseComment(HAPPY_PATH_DEFAULTS, TEST_COMMENT),
        ).toMatch(/^console\.log\('/gi);
      });
      ...
      test('escapes "escaper" char (e.g. "\'") with slash, that is not counted in line length as it will not print on execution', () => {
        const output = outputHelpers.parseComment(
          HAPPY_PATH_DEFAULTS,
          "TEST_'_COMMENT",
        );
        ...
        expect(output).toMatch(/^console\.log\('.{61}'\);$/gi);
      });
      ...
      describe('with passed multiline comment', () => {
        ...
        test('returns multiline output', () => {
          ...
          ).toContain(`* TEST_LINE_1 *`);
          expect(
            outputHelpers.parseComment(
              HAPPY_PATH_DEFAULTS,
              TEST_MULTILINE_COMMENT,
            ),
          ).toContain(`* TEST_LINE_2 *`);
  ...
  describe('has method parseVar', () => {
    ...
    test('returns string as an output', () => {
      expect(
        typeof outputHelpers.parseVar(HAPPY_PATH_DEFAULTS, TEST_VARIABLE),
      ).toBe('string');
    });
    ...
    describe('with default settings', () => {
      test('contains variable passed as parameter', () => {
        expect(
          outputHelpers.parseVar(HAPPY_PATH_DEFAULTS, TEST_VARIABLE),
      ...
      test('returns variable just after console.log prefix on the beginning of string', () => {
        expect(
          outputHelpers.parseVar(HAPPY_PATH_DEFAULTS, TEST_VARIABLE),
        ).toMatch(/^console\.log\('\* TEST_VARIABLE/gi);
      });
      ...
      test('wraps variable with "JSON.stringify(" before it', () => {
        expect(
          outputHelpers.parseVar(HAPPY_PATH_DEFAULTS, TEST_VARIABLE),
        ).toMatch(/JSON.stringify\(TEST_VARIABLE/gi);
      });

      test('wraps variable with ", null, \'\\t\')" after it', () => {
        expect(
          outputHelpers.parseVar(HAPPY_PATH_DEFAULTS, TEST_VARIABLE),
        ).toMatch(/TEST_VARIABLE, null, '\\t'\)/gi);
      });
    });
  });
});
